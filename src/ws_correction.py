    ...
    def move_wrong_scheme_messages(user, data, coda_input_dir):
        ...
        for td in data:
            for plan in PipelineConfiguration.RQA_CODING_PLANS + PipelineConfiguration.FOLLOW_UP_CODING_PLANS + \
                ...
                rqa_and_survey_codes = []
                for cc in plan.coding_configurations:
                    if cc.coding_mode == CodingModes.SINGLE:
                        if f"{cc.coded_field}_WS" in td:
                            label = td[f"{cc.coded_field}_WS"]
                            rqa_and_survey_codes.append(cc.code_scheme.get_code_with_code_id(label["CodeID"]))
                    else:
                        assert cc.coding_mode == CodingModes.MULTIPLE
                ...
                for code in rqa_and_survey_codes:
        ...
        for group in data_grouped_by_uid.values():
            ...
            # (Note: we only need to check one td in this group because all the surveys are the same)
            td = group[0]
            ...
            raw_survey_fields = {plan.raw_field for plan in PipelineConfiguration.DEMOG_CODING_PLANS +
                                 PipelineConfiguration.FOLLOW_UP_CODING_PLANS}
            raw_rqa_fields = {plan.raw_field for plan in PipelineConfiguration.RQA_CODING_PLANS}
            for plan in PipelineConfiguration.DEMOG_CODING_PLANS + PipelineConfiguration.RQA_CODING_PLANS + \
                        PipelineConfiguration.FOLLOW_UP_CODING_PLANS:
